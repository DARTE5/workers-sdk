...
export function* dumpSql(
	...
	// Taken from SQLite shell.c.in https://github.com/sqlite/sqlite/blob/master/src/shell.c.in#L8444-L8450
  ...
  `)();
	...
	for (const { name: table, sql } of tables) {
		...
		} else if (table.startsWith(`sqlite_stat`)) {
		...
		for (const dataRow of rows_cursor.raw()) {
			const formattedCells = dataRow.map((cell: unknown, i: number) => {
				...
				if (cell === null) {
					...
					// Reproduce `output_quoted_escaped_string` from https://github.com/sqlite/sqlite/blob/master/src/shell.c.in#L1799-L1862
					let lfs = false;
					let crs = false;
					// First escape any ' as ''
					let output_string = `'${(cell as string).replace(
						/'|(\n)|(\r)/g,
						(_: string, lf: string, cr: string) => {
							// While also tracking whether the string contains \r or \n
							if (lf) {
								lfs = true;
								return `\\n`;
							}
	...
	if (!noSchema) {
		// Taken from SQLite shell.c.in https://github.com/sqlite/sqlite/blob/master/src/shell.c.in#L8454-L8460
